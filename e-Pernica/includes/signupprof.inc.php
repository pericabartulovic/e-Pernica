...
if (isset($_POST['signup-submit'])) {
    ...
    $username = $_POST['imeprez'];
    $email = $_POST['mail'];
    $password = $_POST['pwd'];
    $passwordPonovi = $_POST['pwd-ponovi'];

    if (/* empty */!($username) || (!$email) || (!$password) || (!$passwordPonovi)) {
        header("Location: ../signupprof.php?error=emptyfields&uid=" . $username . "&mail=" . $email);
        exit();
/*     } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL) && !preg_match("/^[a-zA-Z0-9_-]*$/", $username)) {
        header("Location: ../signup.php?error=invalidmailuid");
        exit(); */
    } elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        header("Location: ../signupprof.php?error=invalidmail&pid=" . $username);
        exit();
/*     } elseif (!preg_match("/^[a-zA-Z0-9]*$/", $username)) {
        header("Location: ../signup.php?error=invaliduid&mail=" . $email);
        exit(); */
    } elseif ($password !== $passwordPonovi) {
        header("Location: ../signupprof.php?error=passwordcheck&pid=" . $username . "&mail=" . $email);
        exit();
    } else {
        $sql = "SELECT imeprez FROM profesori WHERE imeprez=?";
        $upit = $konekcija->prepare($sql);                   // mysqli_stmt_init($konekcija);
        if (!$upit/* mysqli_stmt_prepare($upit, $sql) */) {
            header("Location: ../signupprof.php?error=sqlerror");
            exit();
        } else {
            ...
            if ($resultCheck > 0) {
                header("Location: ../signupprof.php?error=usertaken&mail=" . $email);
                exit();
            } else {
                $sql = "INSERT INTO profesori (imeprez, emailprof, pwdProf) VALUES (?, ?, ?)";
                ...
                if (!$upit/*mysqli_stmt_prepare($upit, $sql)*/) {
                    ...
                    mysqli_stmt_execute($upit);  */
                    header("Location: ../signupprof.php?signup=success");
                    exit();
                }
            }
        }
    ...
    header("Location: ../signupprof.php");
    exit();
}
